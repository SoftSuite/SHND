...
{
    /// <summary>
    /// Represents a transaction for V_ORDERFEED_WAIT_REGISTER view.
    /// [Created by 127.0.0.1 on April,28 2009]
    /// </summary>
    public class VOrderFeedWaitRegisterDAL
    {

        public VOrderFeedWaitRegisterDAL()
        {
        }
        ...
        /// <summary>
        /// Gets the select statement for V_ORDERFEED_WAIT_REGISTER table.
        /// </summary>
        ...
        public DataTable GetDataListByConditions(double cWARD, double cFOODTYPE, string cFOODCATEGORY, string cPATIENTNAME, DateTime cORDERDATEFROM, DateTime cORDERDATETO, DateTime cREGDATEFROM, DateTime cREGDATETO, string cHN, string cVN, string cAN, string orderBy, OracleTransaction trans)
        {
            string whText = "STATUS = 'RG' AND ISFAMILY = 'N' AND REQTYPE = 'B' ";

            if (cWARD != 0) whText += (whText == "" ? "" : "AND ") + "WARDID = " + DB.SetDouble(cWARD) + " ";
            if (cFOODTYPE != 0) whText += (whText == "" ? "" : "AND ") + "FOODTYPEID = " + DB.SetDouble(cFOODTYPE) + " ";
            if (cFOODCATEGORY.Trim() != "") whText += (whText == "" ? "" : "AND ") + "FOODNAME = " + DB.SetString(cFOODCATEGORY.Trim()) + " ";
