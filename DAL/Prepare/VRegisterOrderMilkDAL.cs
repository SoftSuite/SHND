...
namespace SHND.DAL.Prepare
{
    ...
    /// </summary>
    public class VRegisterOrderMilkDAL
    {

        public VRegisterOrderMilkDAL()
        {
        }
        ...
        private string sql_select
        {
            get
            {
                string sql = "SELECT CASE WHEN RANK() OVER(PARTITION BY ADMITPATIENT ORDER BY PATIENTNAME, ORDERMILK) = 1 THEN 1 ELSE 0 END RANK, A.* FROM " + viewName + " A ";
                return sql;
            }
        }
        ...
        public DataTable GetDataListByConditions(DateTime date, double cWARD, double cMILKCATEGORY, string cPATIENTNAME, DateTime cORDERDATEFROM, DateTime cORDERDATETO, string orderBy, OracleTransaction trans)
        {
            string whText = "";
            whText = "STATUS = 'FN'";
            if (date.Year != 1) whText += (whText == "" ? "" : "AND ") +
                "TO_DATE(FIRSTDATE) <= " + DB.SetDate(date) + " AND (TO_DATE(ENDDATE) >= " + DB.SetDate(date) + " OR ENDDATE IS NULL) ";
