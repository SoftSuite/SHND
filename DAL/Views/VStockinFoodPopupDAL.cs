...
{
    /// <summary>
    /// Represents a transaction for V_STOCKIN_FOOD_POPUP view.
    /// [Created by 127.0.0.1 on April,30 2009]
    /// </summary>
    public class VStockinFoodPopupDAL
    {

        public VStockinFoodPopupDAL()
        {
        }

        #region Constant

        /// <summary>V_STOCKIN_FOOD_POPUP</summary>
        private const string viewName = "V_STOCKIN_FOOD_POPUP";
        ...
        /// <summary>
        /// Gets the select statement for V_STOCKIN_FOOD_POPUP table.
        /// </summary>
        ...
        /// <summary>
        /// Returns an indication whether the record of V_STOCKIN_FOOD_POPUP by specified condition is retrieved successfully.
        /// </summary>
        ...
        public DataTable GetDataListByConditions(double cPLAN, double cCLASS, string cMATERIALNAME, string exceptCodeList, string orderBy, OracleTransaction trans)
        {
            string whText = "";
            if (cPLAN != 0) whText += (whText == "" ? "" : "AND ") + "PLANORDER = " + DB.SetDouble(cPLAN) + " ";
            if (cCLASS != 0) whText += (whText == "" ? "" : "AND ") + "CLASSLOID = " + DB.SetDouble(cCLASS) + " ";
            if (cMATERIALNAME.Trim() != "") whText += (whText == "" ? "" : "AND ") + "UPPER(MATERIALNAME) LIKE " + DB.SetString("%" + cMATERIALNAME.Trim().ToUpper() + "%") + " ";
            if (exceptCodeList != "") whText += (whText.Trim() == "" ? "" : " AND ") + "CODE NOT IN (" + exceptCodeList + ") ";
