...
namespace SHND.Flow.Inventory
{
    public class MaterialFeedFlow
    {
        ...
        {
            V_MaterialMasterDAL vDAL = new V_MaterialMasterDAL();
            return vDAL.GetDataListByCondition(Name, Group, OrderText, null);
        }
        ...
        public bool UpdateMaterialNutrient(DataTable dt, string material_master_loid, string UserID)
        {
            ...
            try
            {
                ...
                if (ret)
                {
                    ...
                    if (InsertMaterialNutrient(dt, material_master_loid, trans, UserID))
                    {
                            trans.CommitTransaction();
                            ret = true;
                    }
        ...
        public bool CheckUniqMilk(double cMilk, string cLOID)
        {
            MaterialMasterDAL msDAL = new MaterialMasterDAL();
            msDAL.GetDataByMilk(cMilk, null);
            return !msDAL.OnDB || (cLOID == msDAL.LOID.ToString());
        }
    }
