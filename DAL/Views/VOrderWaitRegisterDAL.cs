...
{
    /// <summary>
    /// Represents a transaction for V_ORDER_WAIT_REGISTER view.
    /// [Created by 127.0.0.1 on April,7 2009]
    /// </summary>
    public class VOrderWaitRegisterDAL
    {

        public VOrderWaitRegisterDAL()
        {
        }
        ...
        private string sql_distinct
        {
            get
            {
                string sql = "SELECT DISTINCT WARDNAME,ROOMNO, BEDNO,HN,AN,VN,PATIENTNAME, ";
                       sql += " AGE,WEIGHT,HEIGHT,BMI, ADMITPATIENT, ORDERMEDICALSETID, ORDERNONMEDICALID,BIRTHDATE ";
                       sql += " FROM V_ORDER_WAIT_REGISTER  ";
                return sql;
            }
        }

        private string sql_ctl
        {
            get
            {
                string sql = " SELECT DISTINCT FOODTYPENAME,CONTROL, ABSTAIN, REMARKS, ";
                       sql += " FOODCATEGORYNAME, QTY, LIMIT, NEED, STATUS, STATUSNAME, ORDERDATE, ";
                       sql += " REGISTERDATE, INCREASE, ADMITPATIENT, ORDERTIME ";
                       sql += " FROM V_ORDER_WAIT_REGISTER ";
                return sql;
            }
        }
        ...
        private string sql_list
        {
            ...
            {
                string sql = "SELECT DISTINCT WARDNAME,ROOMNO, BEDNO,HN,AN,VN,PATIENTNAME, ";
                       sql += " AGE,WEIGHT,HEIGHT,BMI, ADMITPATIENT, ";
                       sql += " FOODTYPENAME,CONTROL, ABSTAIN, REMARKS, ";
                       sql += " FOODCATEGORYNAME, QTY, LIMIT, NEED, STATUS, ORDERDATE, ";
                       sql += " REGISTERDATE, INCREASE, ADMITPATIENT, ORDERTIME ,ORDERNONMEDICALID, ORDERMEDICALSETID ";
                       sql += " FROM V_ORDER_WAIT_REGISTER  ";
                return sql;
            }
        }

        public DataTable GetDataDistinct(string whereClause, string orderBy, OracleTransaction trans)
        {
            return DB.ExecuteTable(sql_distinct + (whereClause == "" ? "" : "WHERE " + whereClause + " ") + (orderBy == "" ? "" : "ORDER BY " + DB.SetSortString(orderBy)), trans);
        }

        public DataTable GetDataCtl(string whereClause, string orderBy, OracleTransaction trans)
        {
            return DB.ExecuteTable(sql_ctl + (whereClause == "" ? "" : "WHERE " + whereClause + " ") + (orderBy == "" ? "" : "ORDER BY " + DB.SetSortString(orderBy)), trans);
        }

        public DataTable GetDistinctDataList(string whereClause, string orderBy, OracleTransaction trans)
        {
            return DB.ExecuteTable(sql_list + (whereClause == "" ? "" : "WHERE " + whereClause + " ") + (orderBy == "" ? "" : "ORDER BY " + DB.SetSortString(orderBy)), trans);
        }
